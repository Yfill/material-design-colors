...
import type { StyleOptions } from '@yfill/theme';
interface ColorMap {
    [color: string]: string[]
}
const colorMap: ColorMap = {
    ...
    "deep-range": ["#FBE9E7", "#FFCCBC", "#FFAB91", "#FF8A65", "#FF7043", "#FF5722", "#F4511E", "#E64A19", "#D84315", "#BF360C", "#FF9E80", "#FF6E40", "#FF3D00", "#DD2C00"],
...
const getStyleOptions = (theme: Theme, color: string, mark: string, defaultIndex = 5): StyleOptions => {
    let colorGroup = colorMap[color];
    if (!colorGroup) throw new Error('The color does not exist');
    const mainColor = colorGroup[defaultIndex];
    colorGroup = [mainColor, ...colorGroup];
    const colorNameMap = ['primary', '50', '100', '200', '300', '400', '500', '600', '700', '800', '900', 'a100', 'a200', 'a400', 'a700'].reduce((result, current, index) => { result[`${index}`] = current; return result; }, {} as { [key: string]: string });
    return {
        mark,
        color: mainColor,
        backgroundColorGroup: colorGroup,
        backgroundColorNameMap: colorNameMap,
        borderColorGroup: colorGroup,
        borderColorNameMap: colorNameMap,
        fontColorGroup: colorGroup,
        fontColorNameMap: colorNameMap
    };
};
...
const changeHandlerMap: { main: Handler } = { main: defaultHandler };
const markList = ['main', 'secondary'];
class MaterialDesignColors {
    ...
    static install(theme: Theme, color: string | string[]): void {
        theme.setMainMDColor = (c: string | string[]) => {
            if (changeHandlerMap.main) theme.off('change', changeHandlerMap.main);
            const colorList = typeof c === 'string' ? [c] : c;
            const getStyleOptList = () => colorList.slice(0, 2).map((c, index) => getStyleOptions(theme, c, markList[index]));
            changeHandlerMap.main = <Handler>(() => { getStyleOptList().forEach((styleOpt) => theme.update(styleOpt)); });
            changeHandlerMap.main();
            return theme.on('change', changeHandlerMap.main);
        };
        theme.setMainMDColor(color);
    }
    static uninstall(theme: Theme): void {
        ...
        markList.forEach((mark) => mark !== 'main' && theme.remove(mark));
        theme.off('change', changeHandlerMap.main);
    }
}
